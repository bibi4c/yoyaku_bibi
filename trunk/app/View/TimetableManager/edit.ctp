...
echo $this->Form->create ( 'Timetable', array (
		'url' => array (
				...
				'action' => 'edit',
				$this->data['Timetable']['timetable_id'] 
		) 
) );
$timetableId = 0;
if(isset($this->data['Timetable']['timetable_id']) && ($this->data['Timetable']['timetable_id'] > 0)){
  $timetableId = $this->data['Timetable']['timetable_id'];
  echo $this->Form->hidden('Timetable.timetable_id',array('value'=>$timetableId)); 
}
?>
<div class='hero-unit1'>
	<dl class='dl-horizontal'>
	   <?php  echo "<h4 style='color: red'><br>※別の時間帯と重複する場合、エラーになります。<br>※時間帯を削除すると、登録したコースと予約したデータも全部削除されます。</h4><br>"?>
    <div class="errorMsg">
	...
	<div style="text-align:left;float:left">		
		<?php  
			echo $this->Html->link(__('削除'), 
				array('action'=>'delete', $this->data['Timetable']['timetable_id']),
